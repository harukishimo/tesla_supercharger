...
| 定期処理 | Vercel Cron + Route Handler | 順番5分前判定、終了3分前確認、呼び出し失効、自動整理 |
...
| `GET /api/cron/process-queue` | 順番5分前Push、終了3分前確認、呼び出し失効、自動完了を処理。Vercel Cronだけが呼ぶ |
...
Vercel Cronから1分ごとに`GET /api/cron/process-queue`を実行する。1分間隔のCronはVercel Pro以上を前提とし、Cron実行はProduction Deploymentだけで行う。
...
- DB接続文字列、OneSignal REST API Key、Vercel Cron用の秘密値など、Vercel server-only環境変数をGitへ含めない。
