...
- DBから有効な待ち列エントリーが存在する施設IDだけを取得し、待ち列が空の施設にはトランザクションを開始しない。
- 対象施設は最大4施設ずつ並行処理し、DB接続数を制限しながら実行時間を短縮する。
- 推定呼び出し時刻の5分前を検出する。
...
| 公開範囲 | public。個人情報を含めず、未認証の再取得合図として扱う |
| 再取得 | event受信後、`get-my-queue`で本人状態を取得 |
| 切断時 | 10〜15秒pollingへ切替 |
| 再接続時 | 現在の`queue_version`を取得し、差分があれば再取得 |

`charging_sites.queue_version`のDB Triggerがpublic Broadcastを送る。private channelに必要なSupabase Authを実装せずに受信できる一方、第三者が偽のイベントを送る可能性があるため、クライアントはpayloadを状態として採用しない。施設IDと更新番号を検査・coalesceしたうえでAPI再取得の合図にだけ使い、APIレスポンスを正本とする。順位、ニックネーム、Push Subscription ID、待ち列ID、管理トークンはBroadcast payloadへ含めない。
