...
| `POST /api/queue/skip-start` | 現地で確認した空きストールで、待機をスキップして充電開始 | `entryId`、管理トークン | 充電中状態、再計算後の待ち列 |
...
|---|---|---|
| `GET /api/cron/process-queue` | 外部スケジューラーのみ | 順番5分前、終了3分前、呼び出し失効、終了予定+5分の自動完了を処理 |
...
- `POST /api/queue/skip-start`は、`waiting`または`notified`の本人だけが利用できる。画面で「周りに並んでいそうな車両はいませんか？」を確認し、問題ないと利用者が明示した場合だけ呼ぶ。施設ロック中に他の`called`・`charging`利用者へ割当済みではないストールを確保し、充電中へ移行して後続を再計算する。
