...
| `CRON_SECRET` | 不可 | 外部スケジューラーから呼ぶCron用エンドポイントの保護 | 定期処理を有効化する時 |
...
- `vercel.json`には`crons`を登録しない。Vercel Hobbyでもデプロイできるようにするためである。
- cron-job.orgなどの無料外部スケジューラーから`GET /api/cron/process-queue`を毎分呼ぶ。
- 呼び出しには`Authorization: Bearer <CRON_SECRET>`ヘッダーを付け、Route Handlerは`CRON_SECRET`と一致する場合だけ処理する。
- Cron Routeは有効なエントリーが存在する施設だけを最大4施設ずつ処理する。待ち列が空なら施設マスタ全件へ接続しない。
- 外部スケジューラーの実行履歴で成功（HTTP 200）と遅延を監視する。URLへsecretを含めない。
...
Vercel HobbyのCronは1日1回に制限されるため、毎分処理は外部スケジューラーへ分離する。cron-job.orgでは無料で1分間隔のHTTP実行とカスタムヘッダーを設定できる。
...
`CRON_SECRET`はVercelと外部スケジューラーの両方へ同じ値を設定する。cron-job.orgのアカウント・実行履歴へsecretが表示されるため、専用secretを使用し、URL・GitHub・スクリーンショットへ記録しない。

GitHub Actionsのscheduleは最短5分で、負荷により遅延する可能性があるため、3分前通知と5分猶予の境界を扱う現在仕様には採用しない。
