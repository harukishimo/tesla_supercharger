...
3. Broadcast payloadは`siteId`と`queueVersion`だけに限定する。
...
3. `vercel.json`にはCronを登録していないことを確認する。Hobbyプランの毎分Cron制限を回避するためである。
4. cron-job.orgで次のJobを作成する。
...
5. JobのTest runでHTTP 200を確認し、VercelのRuntime Logsにも`GET /api/cron/process-queue`が記録されることを確認する。
6. 実行履歴が毎分増え、通常時は外部スケジューラーのTimeout以内にHTTP 200が返ることを確認する。
