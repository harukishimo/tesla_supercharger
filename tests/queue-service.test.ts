...
test("4ストールのうち10分で空く充電枠を先頭待機者へ反映する", async () => {
  const store = new InMemoryQueueStore();
  const fourStallSite = { ...site, id: "00000000-0000-4000-8000-000000000020", stallCount: 4 };
  store.addSite(fourStallSite, [
    emptySlot("slot-a", "unknown"),
    emptySlot("slot-b", "unknown"),
    emptySlot("slot-c", "unknown"),
    emptySlot("slot-d", "unknown"),
  ]);
  setQueueStoreForTests(store);
  try {
    const chargingUser = await joinQueue({ siteId: fourStallSite.id, nickname: "1", siteIsFull: true, acceptedTerms: true, termsVersion: "2026-07-22", now });
    const waitingUser = await joinQueue({ siteId: fourStallSite.id, nickname: "2", siteIsFull: false, acceptedTerms: true, termsVersion: "2026-07-22", now: new Date(now.getTime() + 1_000) });
    await skipWaitAndStartQueue(chargingUser.entryId, chargingUser.token, new Date(now.getTime() + 2_000));
    await setDuration(chargingUser.entryId, chargingUser.token, 10, new Date(now.getTime() + 3_000));
    const recalculated = await getMyQueue(waitingUser.entryId, waitingUser.token, new Date(now.getTime() + 4_000));
    assert.equal(recalculated.estimatedWaitMinutes, 10);
    assert.equal(recalculated.estimatedStartAt, "2026-07-22T06:30:02.000Z");
  } finally {
    setQueueStoreForTests(undefined);
  }
});
