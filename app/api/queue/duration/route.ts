import { setDuration } from "@/lib/server/queue-service";
...
export async function POST(request: Request) {
  return route(async () => {
    ...
    return idempotent("queue:duration", idempotencyKey, { entryId, token, minutes: body.minutes }, () => setDuration(entryId, token, body.minutes, new Date(), { key: idempotencyKey, fingerprint: { entryId, token, minutes: body.minutes } }));
  });
}
