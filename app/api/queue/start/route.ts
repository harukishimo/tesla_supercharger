import { startQueue } from "@/lib/server/queue-service";
...
export async function POST(request: Request) {
  return route(async () => {
    ...
    return idempotent("queue:start", idempotencyKey, { entryId, token }, () => startQueue(entryId, token, new Date(), { key: idempotencyKey, fingerprint: { entryId, token } }));
  });
}
