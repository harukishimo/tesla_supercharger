...
- `CRON_SECRET`（外部スケジューラーからCron Routeを呼ぶ場合）
...
6. Realtime切断時の再取得、失敗時のエラー表示、（有効化した場合）OneSignal通知許可と外部Cronの認証を確認する。
7. Supabaseの監視・Vercel Functionログ・外部スケジューラーの実行履歴に、DB接続文字列、管理トークン、Push APIキーが記録されていないことを確認する。
